...
uids = set()
iids = set()
for userID, itemID in df.iterrows():
    cv = itemID.to_numpy()
    n =0
    for item in cv:
        n=n+1
...
compression_opts = dict(method='zip', archive_name='outCome.csv') 
df_test.to_csv('predictionsResult.zip', index=False, compression=compression_opts)
#sortPredictions =df_test.sort_values(by=['ncf_predictions'], ascending=False)
